...
namespace
{
	...
	float gMinimumMovementSpeed = 0.5f; // this is the minimum speed the camera can move
	...
	ShaderManager *pShaderManager)
{
	...
	g_pCamera->MouseSensitivity = 0.05f;
}
...
{
	// Alter camera movement speed by yOffset
	g_pCamera->MovementSpeed += (float)yOffset;

	// Don't let the camera speed go below a set minimum
	if (g_pCamera->MovementSpeed < gMinimumMovementSpeed)
	{
		g_pCamera->MovementSpeed = gMinimumMovementSpeed;
	}
}
